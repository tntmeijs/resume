...
\vspace{-9pt}
\begin{itemize}
  \item Designed and built components for a serverless (AWS / Go) digital onboarding flow for a large Dutch bank by creating new (micro)services, integrating with existing systems, and designing the application's architecture.
  \item Lead the development of Deloitte Netherlands' cloud native (Java / AWS) digital loan origination system for \href{https://www2.deloitte.com/us/en/pages/consulting/solutions/converge/modern-banking-suite.html}{Converge Banking} by defining capabilities, designing new features, and improving the loan origination journey.
  ...
  \item Improved the engineering team's talent pool by taking part in a few dozen hiring interviews to make sure that we hire the right profiles that would complement the team's existing capabilities.
...
\begin{itemize}
  ...
  \item Built brand new frontends for various applications by using React and JavaScript so we could continue to deliver value as a team while searching for a new frontend developer in the meantime.
\end{itemize}

\textit{Junior Software Engineer} \\
\vspace{-9pt}
\begin{itemize}
  \item Built a digital loan origination solution for a large Dutch bank that reduces the time it takes to apply for a loan from 6 weeks to 15 minutes by automating their offline processes.
